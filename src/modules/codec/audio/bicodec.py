...
try:
    ...
    if bool(os.getenv("ACHATBOT_PKG", "")):
        ...
        sys.path.insert(1, os.path.join(cur_dir, "../../../../deps/F5TTS"))
...
class BiCodecTokenizer(EngineClass, ICodec):
    ...
    def load_model(self):
        ...
        self.model = bicodec.BiCodec.load_from_checkpoint(f"{self.model_dir}/BiCodec").to(
            self.args.device
        )
    ...
    def encode_code(self, waveform_tensor: torch.Tensor) -> torch.Tensor:
        ...
        batch = {
            ...
            "ref_wav": ref_waveform_tensor.to(self.args.device),
            "feat": feat.to(self.args.device),
        }
        semantic_tokens, global_tokens = self.model.tokenize(batch)

        return torch.stack((semantic_tokens, global_tokens))

    def decode_code(self, tokens: torch.Tensor) -> torch.Tensor:
