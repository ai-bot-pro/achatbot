...
def get_system_prompt(**kwargs) -> str:
    ...
    if len(args.roles) < 2:
        raise Exception("roles number must >=2")
    roles = []
    for i in range(0, len(args.roles)):
        roles.append(f"Role{i} as {args.roles[i]}")
...
[Only display the conversation in your output, using advanced TTS-specific markup as needed. Example:
...
[Extract podcast roles. For each podcast role, provide a name, content.]
exact_flow:
```
...
[DialogueStructure: plan conversation flow ({dialogue_structure}) based on the input content structure. Start the conversation greeting the audience listening also saying "welcome to {args.podcast_name}  - {args.podcast_tagline}." End the conversation greeting the audience with Role1 also saying a good bye message. ]
...
class Role(BaseModel):
    ...
    content: str = Field(
        ...,
        description="the podcast's content, don't use words like 'the speaker'",
    )
