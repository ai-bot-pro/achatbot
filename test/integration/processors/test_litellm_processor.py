...
from src.types.frames.control_frames import UserImageRequestFrame, UserStartedSpeakingFrame, UserStoppedSpeakingFrame
...
LITELLM_MODEL="github/gpt-4o" \
    python -m unittest test.integration.processors.test_litellm_processor.TestProcessor.test_run_text
...
class TestProcessor(unittest.IsolatedAsyncioTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.model = os.getenv('LITELLM_MODEL', "github/gpt-4o")
    ...
    async def describe_image(
        ...
        if self.max_function_call_cn > self.get_weather_call_cn:
            ...
            await llm.push_frame(
                UserImageRequestFrame(self.uid), FrameDirection.UPSTREAM)
        else:
    ...
    async def test_run_text(self):
        ...
        print("llm_context.get_messages-->", msgs)
