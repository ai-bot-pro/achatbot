...
class imageResponseAggregator(FrameProcessor):
    def __init__(
        self,
        *,
        start_frame: Type[Frame],
        end_frame: Type[Frame],
        accumulator_frame: Type[ImageRawFrame],
        interim_accumulator_frame: Type[ImageRawFrame] | None = None,
    ):
        super().__init__()
    ...
    def _reset(self):
        ...
        self._aggregation: list[ImageRawFrame] = []
        self._aggregating = False
        self._seen_start_frame = False
        self._seen_end_frame = False
        self._seen_interim_results = False
        self._cur_image_frame = None
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, self._start_frame):
            ...
            # aggregating (i.e. we have seen interim results but not the final image
            ...
            if self._aggregating:
                self._cur_image_frame = frame
                self._aggregation.append(frame)
    ...
    async def _push_aggregation(self):
        if len(self._aggregation) > 0:
            await self.push_frame(VisionImagesRawFrame(images=self._aggregation))

            # Reset our accumulator state.
            self._reset()


class UserimageResponseAggregator(imageResponseAggregator):
    """
    user input image frame aggregator, no history, just aggr image bytes
    """
    ...
    def __init__(self):
        super().__init__(
            ...
            accumulator_frame=ImageRawFrame,
        )
