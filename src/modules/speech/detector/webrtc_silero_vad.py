...
class WebrtcSileroVAD(BaseVAD):
    ...
    def __init__(self, **args) -> None:
        ...
        self.webrtc_vad = WebrtcVAD(**WebRTCVADArgs(
        ...
        self.silero_vad = SileroVAD(**SileroVADArgs(
            sample_rate=self.args.sample_rate,
    ...
    async def detect(self, session: Session):
        ...
        if is_webrtc_speech is False:  # fast path
            return False
        logging.debug(f"webRTC has detected a possible speech")
        ...
        if is_silero_speech is False:
            logging.debug(f"silero VAD has detected a noise")
            return False
        logging.debug(f"silero VAD has detected a possible speech")
        return True
