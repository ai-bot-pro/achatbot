...
@app.command()
def offline_stream():
    ans = pipeline(Tasks.acoustic_noise_suppression, model="iic/speech_frcrn_ans_cirm_16k")
    ...
    window = 1 * 16000 * 2  # 2 秒的窗口大小，以字节为单位
    outputs = b""
    total_bytes_len = 0
    audiostream.read(44)
    ...
    output_path = "frcrn_enhancer_output_stream.wav"
...
def offline():
    ...
    result = ans(
        ...
        output_path="frcrn_enhancer_output.wav",
    )
    print(len(result["output_pcm"]))


"""
python demo/modelscope/ans_frcrn_enhancer.py offline
python demo/modelscope/ans_frcrn_enhancer.py offline-stream
"""
