...
TOKENIZERS_PARALLELISM=false python -m src.cmd.bots.main -f config/bots/agora_asr_translate_llamacpp_tts_bot.json

# 2. run webrtc room http signal bot server
TOKENIZERS_PARALLELISM=false python -m src.cmd.http.server.fastapi_room_bot_serve -f config/bots/agora_asr_translate_llamacpp_tts_bot.json
# AgoraASRTranslateTTSBot join chat-room
curl -XPOST "http://0.0.0.0:4321/bot_join/chat-room/AgoraASRTranslateTTSBot"

# 3. run webrtc room http bots server (experimental)
TOKENIZERS_PARALLELISM=false python -m src.cmd.http.server.fastapi_agora_bot_serve
# AgoraASRTranslateTTSBot join chat-room with bot config
curl --location 'http://0.0.0.0:4321/bot_join/chat-room/AgoraASRTranslateTTSBot' \
--header 'Content-Type: application/json' \
--data '{
    "chat_bot_name": "AgoraASRTranslateTTSBot",
    "room_name": "chat-room",
    "room_url": "",
    "token": "",
    "room_manager": {
...
class AgoraASRTranslateTTSBot(AgoraChannelBot):
    """
    agora transport(webrtc) with vad -> asr -> translate LLM -> punc -> tts
    """
    ...
    async def arun(self):
        ...
        #    prefix_name="agora_asr_translate_tts_bot",
    ...
    async def on_first_participant_say_hi(self, transport: AgoraTransport, user_id: int):
        self.session.set_client_id(user_id)
