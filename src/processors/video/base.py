...
class VideoGenProcessor(AIProcessor):
    ...
    def set_gen_video_frame(self, frame: TextFrame):
        self._gen_video_frame = frame
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, self._gen_video_frame):
            if self._is_pass_frame:
                await self.push_frame(frame, direction)
            await self.start_processing_metrics()
            await self.process_generator(self.run_video_gen(frame.text))
