...
from src.common.utils.audio_utils import load_json, get_audio_segment, bytes2TorchTensorWith16
    ...
    def test_detect_activity(self):
        ...
        for audio_file, data in annotations.items():
            ...
            for annotated_segment in data["segments"]:
                ...
                adjusted_vad_res = [{"start": segment["start"] + annotated_segment["start"],
                                     "end": segment["end"] + annotated_segment["start"]}
                                    for segment in vad_res]

                detected_segments = [segment for segment in adjusted_vad_res if
