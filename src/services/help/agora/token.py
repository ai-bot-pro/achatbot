        ...
        for _, item in access_token._AccessToken__service.items():
            for key, serviceItem in item.__dict__.items():
                if key == '_Service__type':
                    ...
                    key_name = key.replace('_Service%s__' %
                                TokenPaser.service[item._Service__type]['name'], '')
                    val = serviceItem.decode() if type(serviceItem) == bytes else serviceItem
                    match sType:
                        case 1:
            ...
            for key, serviceItem in item.__dict__.items():
                if key == '_Service__type':
                    ...
                    res += '- Service information, type:%d (%s) \n' % (
                        item._Service__type, TokenPaser.service[item._Service__type]['name'])
                elif key == '_Service__privileges':
                    for privilege, privilegeExpire in item._Service__privileges.items():
                        res += '    - privilege:%d(%s), expire:%d (%s) \n' % (
                            privilege, TokenPaser.service[item._Service__type]['privilege'][privilege]['name'], privilegeExpire,
                            TokenPaser.get_expire_msg(access_token._AccessToken__issue_ts + privilegeExpire))
                else:
                    res += '    - {}:{} \n'.format(key.replace('_Service%s__' % TokenPaser.service[item._Service__type]['name'], ''),
